...
using ..bases, ..states, ..sparsematrix2
...
sparse_identity(b1::Basis, b2::Basis) = SparseOperator(b1, b2, sparsematrix.speye(Complex128, length(b1), length(b2)))
...
function operators.gemm!{T<:Complex}(alpha::T, M::SparseOperator, b::Operator, beta::T, result::Operator)
    sparsematrix2.gemm!(alpha, M.data, b.data, beta, result.data)
end
function operators.gemm!{T<:Complex}(alpha::T, a::Operator, M::SparseOperator, beta::T, result::Operator)
    sparsematrix2.gemm!(alpha, a.data, M.data, beta, result.data)
end
function operators.gemv!{T<:Complex}(alpha::T, M::SparseOperator, b::Ket, beta::T, result::Ket)
    sparsematrix2.gemv!(alpha, M.data, b.data, beta, result.data)
end
function operators.gemv!{T<:Complex}(alpha::T, b::Bra, M::SparseOperator, beta::T, result::Ket)
    sparsematrix2.gemv!(alpha, b.data, M.data, beta, result.data)
end
