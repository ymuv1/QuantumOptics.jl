...
tout, ρt4 = stochastic.master(T, ψ0, H, J, 0 .*J; rates=rates, nonlinear=false, dt=dt)
tout, ρt5 = stochastic.master(T, ρ0, LazyProduct(H, one(H)), sqrt.(rates).*J, 0 .* J; Hs=0 .* Hs, nonlinear=false, dt=dt)
tout, ρt6 = stochastic.master_dynamic(T, ρ0, fdeterm_master, fstoch1_master; rates=rates, nonlinear=false, dt=dt)
for i=1:length(tout)
...
# Linear version
tout, ρt = stochastic.master(T_short, ρ0, H, J, 0.*J; nonlinear=false, dt=dt)
tout, ρt = stochastic.master(T_short, ψ0, H, [sm, sm], [sm, sm]; rates=rates_mat, nonlinear=false, dt=dt)
tout, ρt = stochastic.master(T_short, ρ0, H, J, J; rates_s=[0.1], Hs=Hs, nonlinear=false, dt=dt)
